...
public class GenerateBehaviour extends CyclicBehaviour {
	...
	public void action() {
		...
		if (message != null)
		{
			...
			switch(request)
			{
			case "create": Action addaction = new Action();					
				switch(object)
				{
				case "start event" : 			
			...
			case "remove" : Action removeaction = new Action();//A noter que les fct removes sont en fait  removeWithId() et non removeWithLabel()
			//factoriser ttes les methodes remove en removeItemWithID() removeItemWithLabel()
				switch(object)
				{
				case "start event" :  
			...
			case "connect": Action connectaction = new Action();
					String[] param = args.split(",");
					connectaction.setType(request);
					for(int i=0;i<param.length;i++)connectaction.addId(param[i]);
	...
	private void GeneratePool(String type, String args, Action act) {
		String[] param = args.split(",");
		Pool p = new Pool(param[0],Integer.parseInt(param[1].trim()),Integer.parseInt(param[2].trim()),Integer.parseInt(param[3].trim()));
		act.setType(type);
		act.addObject(p);
	}
