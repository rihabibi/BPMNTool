...
public class WorkFlow {
	...
	//renvois la valeur de la colone la plus basse (sachant qu'elle peut etre nï¿½gative) le min est forcement <=0
	...
	public void place(ArrayList<ArrayList<ObjectBPMN>> mat)
	{
		...
		for(int i=0;i<H_ligne.size();i++)// parcour des lignes
		{
			...
			for(int j=0;j<Col_taille.size();j++)// parcour des colones
			{
				...
				o.setY(posy+(H_ligne.get(i)/2)-(o.getH()/2)-ecart_H/2); // le -ecart_H/2 permet de compenser l'ï¿½cart du haut tout en restant centrï¿½
				}
		...
		// choix des prioritï¿½s sur l'afffichage des liens dans une mï¿½me colone
